    ...
    options: z
      .object({
        ...
        presence: z
          ...
          .optional()
          .describe("Presença a mostrar"),
        quotedMessageId: z
          .string()
          .optional()
          .describe("ID da mensagem a ser citada"),
      })
      .optional()
      .describe("Opções adicionais para o envio"),
  },
...
server.tool(
  "sendMedia",
  {
    number: z
      .string()
      .min(1)
      .describe("Número do destinatário no formato internacional"),
...
server.tool(
  "checkWhatsAppNumber",
  {
    phone: z
      ...
      .min(1)
      .describe(
        "Número a ser verificado no formato internacional (ex: 5511999999999)",
      ),
  },
  async ({ phone }) => {
    try {
...
server.tool(
  "markMessageAsRead",
  {
    messageId: z
      .string()
      .min(1)
      .describe("ID da mensagem a ser marcada como lida"),
  },
  async ({ messageId }) => {
    try {
...
server.tool(
  "configureEvolutionWebhook",
  {
    webhookUrl: z
      .string()
      .url()
      .describe("URL do webhook (ex: http://lifeos-agent:8000/webhook)"),
    enabled: z
      .boolean()
      .default(true)
      .describe("Habilitar ou desabilitar o webhook"),
    events: z
      .array(
        z.enum([
          "messages.upsert",
      ...
      .optional()
      .describe(
        "Eventos para escutar (padrão: messages.upsert, messages.update, send.message, connection.update)",
      ),
  },
        ...
        contents: [
          {
            ...
            text: `Contatos disponíveis (${contacts.length}):\n${contacts
              .map(
                (contact: any) =>
                  `- ${contact.name || "Sem nome"}: ${contact.id.replace("@c.us", "")}`,
              )
              .join("\n")}`,
          },
        ],
        ...
        contents: [
          {
            ...
            text: `Conversas disponíveis (${chats.length}):\n${chats
              .map(
                (chat: any) => `- ${chat.name || chat.id || "Chat sem nome"}`,
              )
              .join("\n")}`,
          },
        ],
        ...
        contents: [
          {
            ...
            text: `Grupos disponíveis (${groups.length}):\n${groups
              .map(
                (group: any) =>
                  `- ${group.subject || group.id || "Grupo sem nome"} (${group.participants?.length || 0} membros)`,
              )
              .join("\n")}`,
          },
        ],
